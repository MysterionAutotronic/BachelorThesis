...
\begin{document}
    ...
    \appendix
        ...
        \section{Repository Overview}\label{app:repos-overview}
            \noindent\emph{Note.} The following template repositories are included for completeness.
            ...
            \begin{longtable}{@{}lP{10cm}@{}}
                ...
                Repository & Scope \\
                \midrule
                \ghrepo{BachelorThesis_TenantFE}     & Next.js tenant frontend (per-workspace) \\
                \ghrepo{BachelorThesis_TenantBE}     & Express tenant API (+ in-memory cache) \\
                \ghrepo{BachelorThesis_ConfigSchema} & JSON schema / validation for \texttt{config.json} \\
                \ghrepo{BachelorThesis_DashboardFE}  & Central operator/tenant dashboard (Next.js) \\
                \ghrepo{BachelorThesis_DashboardBE}  & Onboarding + pipeline triggers (Express) \\
                \ghrepo{BachelorThesis_Infra}        & KCP / cluster manifests and automation \\
                \addlinespace
