...
\begin{document}
    ...
    \section{Introduction}\label{sec:introduction}
        ...
        \subsection{Objectives and Scope}\label{subsec:objectivesAndScope}
            ...
            \subsubsection{Scope}\label{subsubsec:scope}
                ...
                Security hardening, multi-\gls{az} failover and comprehensive
                \gls{rbac} policies are out of scope.
    ...
    \section{Fundamentals}\label{sec:fundamentals}
        ...
        \subsection{Kubernetes and Multi-Tenancy}\label{subsec:k8sAndMultiTenancy}
            ...
            \subsubsection{The Challenges of Multi-Tenancy and the Need for Solutions}\label{subsubsec:challenges}
                ...
                A secure solution, keeping multi-tenancy's advantages while also addressing security
            ...
            \subsubsection{Kubernetes Control Plane (KCP) as a Promising Approach}\label{subsubsec:kcpApproach}
                ...
                \begin{enumerate}[label={[\arabic*]:},
                    ...
                    \item\label{chal:api}
                        \textit{API}.
                        \gls{kcp} offers a \gls{k8s@gls}-like \gls{api} that enables the use of
            ...
            \subsubsection{Background: The Evolution of Kubernetes}\label{subsubsec:k8sEvolution}
                ...
                supporting everything from conventional applications into emerging
        ...
        \subsection{Kubernetes Control Plane (KCP)}\label{subsec:kcp}
            ...
            \subsubsection{Workspaces}\label{subsubsec:workspaces}
                ...
                organization makes workspaces an attractive solution to many of the problems
            ...
            \subsubsection{API}\label{subsubsec:api}
                ...
                \gls{url}, enabling safe cross-workspace
        ...
        \subsection{SaaS Architecture and Automation}\label{subsec:saas}
            ...
            \begin{enumerate}[label={[\arabic*]:},
                    ...
                    \item\label{chal:saasOperationalEfficiency}
                        ...
                        operational efficiency that unifies tooling, enables rapid
    ...
    \section{State of the Art and Related Work}\label{sec:related}
        ...
        \subsection{Multi-Tenancy Concepts in the Cloud}\label{subsec:mtConcepts}
            As described in \autoref{subsubsec:mtImportance}, a multi-tenant solution is one
    ...
    \section{Conceptual Design}\label{sec:concept}
        ...
        \subsection{Proposed Scenario}\label{subsec:scenario}
            ...
            lightweight yet realistic \textbf{case study}:
            \\
            ...
            \subsubsection{Representative User Journey}\label{subsubsec:userJourney}
                ...
                \gls{e2e} journey from the perspective of the primary user: the \gls{smb} tenant.
        ...
        \subsection{System Requirements}\label{subsec:requirements}
            ...
            \subsubsection{Functional Requirements}\label{subsubsec:fr}
                ...
                The prototype offers only a handful of \gls{e2e} user journeys
                ...
                It focuses on \gls{crud} operations for tenant data, delivery of the public page,
                ...
                \begin{table}[H]\label{tab:fr}
                    ...
                    \begin{tabular}{lcp{4cm}P{8cm}}
                        ...
                        \hypertarget{f7}{F-07} & \textit{Tenant} & Config Caching & The per-tenant \gls{api} shall cache configuration in-memory with a \gls{ttl}~$\geq$~24~hours, falling back to the \gls{json} file on cache miss. \\
                        \bottomrule
                    \end{tabular}
            ...
            \subsubsection{Rationale for Config Injection Strategy}\label{subsubsec:rationale}
                ...
                \begin{enumerate}[label={[\arabic*]:},
                    ...
                    \item\label{chal:configMap}
                        ...
                        and is not designed for cross-workspace use.  
    ...
    \clearpage
            ...
            \subsubsection{Tenant Workspace: Data Plane Components}
                ...
                \begin{enumerate}[label={[\arabic*]:},
                    ...
                    \item\label{chal:db}
                        \textit{Data sources}.
                        The tenant \gls{api} periodically renews its cache based on a \gls{ttl}.
            ...
            \subsubsection{Scalability, Isolation and Security}\label{scalabilityIsolationSecurity}
                ...
                Collectively the security measures have to fulfill \hyperlink{nf7}{NF-07}.\\
        ...
        \subsection{Automated Deployment Strategies}\label{subsec:deploymentStrategies}
            ...
            \subsubsection{Initial Tenant Deployment Pipeline}\label{subsubsec:initialDeeployment}
                ...
                Every artifact is generated from code that lives in Git, every cluster mutation is
    ...
    \section{Prototypical Implementation}\label{sec:prototype}
        ...
        \subsection{Purpose and Reading Guide}\label{subsec:purposeAndReadingGuide}
            ...
            The \gls{dod} for the prototype was one \gls{e2e} tenant lifecycle as described in
    ...
    \clearpage
        ...
        \subsection{Core Components}\label{subsec:coreComponents}
            ...
            \subsubsection{Tenant Frontend}\label{subsubsec:tenantFrontend}
                ...
                This is shown under~\autoref{lst:tenantfe-dockerfile}.
            ...
            \subsubsection{Dashboard API}\label{subsubsec:dashboardAPI}
                ...
                This is shown under~\autoref{lst:dashboardbe-dockerfile}.\\
    ...
    \clearpage
        ...
        \subsection{Challenges, Bugs and Fixes}\label{subsec:challengesAndBugs}
            ...
            \subsubsection{CORS}\label{subsubsec:challengesCORS}
                ...
                This avoids the browser's \gls{cors} checks entirely.
        ...
        \subsection{Lessons Learned}\label{subsec:evalLessonsLearned}
            ...
            \begin{enumerate}[label={[\arabic*]:},
                ...
                \item\label{chal:evalLessonsCORS}
                    ...
                    Moving requests behind server-side proxy routes in \gls{next} (\texttt{/api/...})%chktex 11
    ...
    \appendix
        ...
        \clearpage
            ...
            \subsection{Key Files}
                ...
                \codesummary
                    ...
                    {\gls{express} app with \gls{json} parsing.\@\texttt{GET /config} reads \texttt{data/config.json}, parses and caches it via \texttt{node-cache} (TTL 24 hours).\@\texttt{GET /crash} exits the process. Listens on \texttt{PORT} (default 3000).}
        ...
        \clearpage
                ...
                \codesummary
                    ...
                    {Reads \texttt{username} from form data. If missing, returns an error. Otherwise sets an \gls{http}-only \texttt{user} cookie (24 hours) and redirects to \texttt{/}.}
        ...
        \clearpage
                ...
                \codesummary
                    ...
                    {Navigates to \texttt{/dashboard} via \gls{next}\texttt{useRouter()}.}% chktex 36
        ...
        \clearpage
                ...
                \codesummary
                    ...
                    {\Gls{bash} script that requires an \gls{api} \gls{url} as build argument. Builds the \gls{fe}.}
        ...
        \clearpage
                ...
                \codesummary
                    ...
                    {Creates a \texttt{Deployment} of the \gls{next} app on port 3000 and a NodePort \texttt{Service} (80 -> 3000); sets \texttt{CONFIG\_ENDPOINT} to \texttt{http://dashboard-be.default.svc.cluster.local/config}.}
        ...
        \clearpage
                ...
                \codesummary
                    ...
                    {Defines two \gls{k8s} Ingresses: \texttt{tenant-fe-ingress} routes \texttt{tenant.local} to Service \texttt{tenant-fe}:80, \texttt{tenant-be-ingress} routes \texttt{api.tenant.local} to Service \texttt{tenant-be}:80. Requires \texttt{ingress-nginx}.}
                    {Provides stable edge entrypoints and decouples external \gls{dns} from in-cluster Service details.}
        ...
        \clearpage
                ...
                \begin{listing}[H]
                    \caption{Restore kind context script: \texttt{/scripts/restore-kind-context}}\label{lst:restore-kind-context}
                    \inputminted[
                        fontsize=\small,
                        linenos,
                        breaklines,
