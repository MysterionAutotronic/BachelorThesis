...
\begin{document}
    ...
    \section{Fundamentals}
        ...
        \subsection{Kubernetes and Multi-Tenancy}\label{subsec:k8sAndMultiTenancy}
            ...
            \subsubsection{Kubernetes Control Plane (KCP) as a Promising Approach}\label{subsubsec:kcpApproach}
                KCP offers three capabilities that map cleanly onto today's multi-tenancy pain
                points.
                ...
                \begin{enumerate}[label={[\arabic*]:},
                    ...
                    \item\label{chal:workspaces}
                        \textit{Workspaces}.
                        KCP achieves strong resource isolation through the concept of
        ...
        \subsection{Kubernetes Control Plane (KCP)}\label{subsec:kcp}
            KCP is \enquote{An open source horizontally scalable control plane for Kubernetes-like
            APIs} \parencite{kcpio}.
            % nguyen 2022
            % KCP docs

            \subsubsection{Workspaces}\label{subsubsec:workspaces}
                KPC introduces the concept of \textit{workspaces} to implement multi-tenancy.
                In KCP, a workspace is a Kubernetes-cluster-like HTTPS endpoint exposed under
                ...
                etcd is the primary datastore used by KCP and K8s
                ...
                This allows for resource isolation in KCP.\@
                A \textit{Custom Resource Definition} \textbf{(CRD)} is a declaratively specified
                schema that registers a new API resource, defined by its group, version, kind, and
                OpenAPI schema, into the Kubernetes control plane so the native API server stores
                and serves the objects as first-class resources \parencite{kubernetesCRD}.
                \\
                ...
                dedicated cluster, yet operators manage only a single shared KCP control plane.
            
            \subsubsection{Automation}\label{subsubsec:automation}
    ...
    \section*{List of Figures}\label{sec:lof}
