    ...
    \section{Conceptual Design}\label{sec:concept}
        ...
        \subsection{Architecture Design with KCP for SaaS}\label{subsec:architectureDesign}
            ...
            \subsubsection{Overview and Design Principles}\label{subsubsec:architectureOverview}
                ...
                The platform must host potentially thousands of small, largely independent sites,
                each with mostly predictable, read-heavy traffic and sporadic writes
                (\see{subsec:scenario}).
                Two architectural principles follow immediately:
                \begin{enumerate}[label={[\arabic*]:},
                    ref=Challenge~\arabic*,
                    leftmargin=*,
                    itemsep=0.6\baselineskip]
                    ...
                    \item\label{chal:pushState}
                        ...
                        All \textit{public} traffic should terminate inside the tenant workspace
                ...
                These principles translate into the logical layout in (TODO: figure).

            \subsubsection{Workspace Topology and Multi-Tenancy model}
                ...
                \begin{table}[H]\label{tab:layersOverview}
                    ...
                    \begin{tabular}{lcp{4cm}P{8cm}}
                        ...
                        \midrule
                        Parent (control) & Workspace (\texttt{root}) & Deployment / dashboard, dashboard-\gls{api}, object store & 1 (static) \\
                        Tenant (data) & Workspace (\texttt{root:<tenant-id>}) & Deployment / Frontend, tenant-\gls{api}, stateful set \\
                        \bottomrule
                    \end{tabular}
                    \caption{Layers overview}
                \end{table}

                Each workspace is created via the Workspace \gls{crd} exposed by \gls{kcp}'s
                \gls{api}.
                Creating a tenant in the Dashboard in turn creates a new \gls{kcp} workspace for the
                tenant that is automatically provisioned.
                Deleting a workspace tears down all associated resources.
            ...
            \subsubsection{Parent Workspace: Control Plane Components}\label{subsubsec:controlPlaneComponents}
                ...
                \begin{table}[H]\label{tab:parentComponents}
                    ...
                    \begin{tabular}{lp{4cm}P{8cm}}
                        ...
                        Object Store & TODO & holds \texttt{tenant-id/data.json} \\
                        \bottomrule
                    \end{tabular}
            ...
            \subsubsection{Data Synchronization and Caching Strategy}\label{syncAndCache}
                The tenant \gls{api} polls the parent object store and caches the tenant data with a
                \gls{ttl} in memory.
                Optionally the parent \gls{api} sends an event to the tenant \gls{api} on updates,
                instantly invalidating the cache.
                Because of the lightweight nature of the cached data a cache separate from the
                \gls{api} like a \gls{redis} instance is not required, because the overhead it
                introduces likely doesn't outweigh the benefits.
                \\
                By caching tenant-specific data within the tenant \gls{api} and only polling the
                parent \gls{api} on cache misses, the system effectively decouples the tenant
                \gls{api} from the parent \gls{api}.
                This design significantly improves resilience during traffic spikes and fan-out
                scenarios by reducing direct dependency and shielding the parent \gls{api} from
                high-frequency access patterns as choosing a long \gls{ttl} around 24 h for the
                common case dramatically reduces load on the parent \gls{api}.
